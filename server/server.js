...
/* ════════════════════════════════════════
   GET /api/holidays
   Фильтры: search, month (вычисляется из поля date), limit, offset
════════════════════════════════════════ */
app.get('/api/holidays', validateQuery(HolidayQuerySchema), async (req, res) => {
  ...
  // Реальные поля коллекции
  params.set('fields', 'id,title,subtitle,date,description,people,region,image');
  params.set('sort', 'date');
  params.set('meta', 'total_count');

  if (search) params.set('filter[title][_icontains]', search);
  // month фильтр: date хранится как YYYY-MM-DD, используем _month Directus-фильтр
  if (month !== undefined) params.set('filter[date][_month][_eq]', month);

  try {
    const r = await fetch(`${DIRECTUS_URL}/items/holidays?${params.toString()}`, { headers: directusHeaders });
    const json = await r.json();
    res.set('Cache-Control', 'public, max-age=300');
    res.json({ data: json.data || [], meta: json.meta || {} });
  } catch (err) {
...
app.get('/api/holidays/:id', validateParams(IdParamSchema), async (req, res) => {
  ...
  try {
    const r = await fetch(
      `${DIRECTUS_URL}/items/holidays/${id}?fields=id,title,subtitle,date,description,full_description,people,region,image,tags`,
...
/* ════════════════════════════════════════
   POST /api/proposals
════════════════════════════════════════ */
app.post('/api/proposals', submitLimiter, validateBody(ProposalSchema), async (req, res) => {
